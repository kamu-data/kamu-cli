...
SELECT 'dev.kamu.domain.datasets.DatasetEntryIndexer', 'dev.kamu.domain.core.services.DatasetService', 0
WHERE NOT EXISTS (SELECT *
                  FROM outbox_message_consumptions
                  WHERE consumer_name = 'dev.kamu.domain.datasets.DatasetEntryIndexer');

-- Skip previous `DatasetLifecycleMessage`'s because the structure has changed.
-- This does not cause problems, because `DatasetEntryIndexer` will fill with rows based on current datasets.

...
// TODO: Use derive macro
#[macro_export]
macro_rules! impl_generic_enum_with_variants {
    ($typ:ident<$gen:ident: $trait:ident>) => {
        impl<$gen: $trait> EnumWithVariants<$typ<$gen>> for $typ<$gen> {
            #[inline]
            fn is_variant<V: VariantOf<Self>>(&self) -> bool {
                V::is_variant(self)
            }
...
// TODO: Make a derive macro
#[macro_export]
macro_rules! impl_generic_enum_variant {
    ($enum_type:ident :: $variant:ident ( $variant_type:ident<$gen:ident: $trait:ident> )) => {
        impl<$gen: $trait> Into<$enum_type<$gen>> for $variant_type<$gen> {
            #[inline]
            fn into(self) -> $enum_type<$gen> {
                $enum_type::$variant(self)
            }
        }

        impl<$gen: $trait> VariantOf<$enum_type<$gen>> for $variant_type<$gen> {
            #[inline]
            fn is_variant(e: &$enum_type<$gen>) -> bool {
                match e {
                    $enum_type::<$gen>::$variant(_) => true,
                    _ => false,
                }
            }

            #[inline]
            fn into_variant(e: $enum_type<$gen>) -> Option<Self> {
                match e {
                    $enum_type::<$gen>::$variant(v) => Some(v),
                    _ => None,
                }
            }

            #[inline]
            fn as_variant(e: &$enum_type<$gen>) -> Option<&Self> {
                match e {
                    $enum_type::<$gen>::$variant(v) => Some(v),
                    _ => None,
                }
            ...
            #[inline]
            fn as_variant_mut(e: &mut $enum_type<$gen>) -> Option<&mut Self> {
                match e {
                    $enum_type::<$gen>::$variant(v) => Some(v),
                    _ => None,
                }

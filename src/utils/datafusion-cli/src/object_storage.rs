...
pub async fn get_s3_object_store_builder(
    ...
    if let (Some(access_key_id), Some(secret_access_key)) = (
        // These options are datafusion-cli specific and must be removed before passing through to
        // datafusion. Otherwise, a Configuration error will be raised.
...
pub fn get_gcs_object_store_builder(
    ...
    if let Some(application_credentials_path) = cmd.options.remove("application_credentials_path") {

...
/// This struct encapsulates AWS options one uses when setting up object
/// storage.
...
/// This struct encapsulates GCP options one uses when setting up object
/// storage.
...
mod tests {
    ...
    async fn s3_object_store_builder_default() -> Result<()> {
        ...
        let expected_region =
            Some(std::env::var("AWS_REGION").unwrap_or_else(|_| "eu-central-1".to_string()));
    ...
    async fn s3_object_store_builder_allow_http_error() -> Result<()> {
        ...
        assert_eq!(
            ...
            "Invalid or Unsupported Configuration: Invalid endpoint: http://endpoint33. HTTP is \
             not allowed for S3 endpoints. To allow HTTP, set 'aws.allow_http' to true"
        );
    ...
    async fn s3_object_store_builder_overrides_region_when_resolve_region_enabled() -> Result<()> {
        ...
        let builder = get_s3_object_store_builder(table_url.as_ref(), &aws_options, true).await?;
    ...
    async fn oss_object_store_builder() -> Result<()> {
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET OPTIONS('aws.access_key_id' \
             '{access_key_id}', 'aws.secret_access_key' '{secret_access_key}', 'aws.oss.endpoint' \
             '{endpoint}') LOCATION '{location}'"
        );
    ...
    async fn gcs_object_store_builder() -> Result<()> {
        ...
        let scheme = table_url.scheme();
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET OPTIONS('gcp.service_account_path' \
             '{service_account_path}', 'gcp.service_account_key' '{service_account_key}', \
             'gcp.application_credentials_path' '{application_credentials_path}') LOCATION \
             '{location}'"
        );
    ...
    async fn check_aws_envs() -> Result<()> {
        ...
        for aws_env in aws_envs {
            std::env::var(aws_env)
                .map_err(|_| exec_datafusion_err!("aws envs not set, skipping s3 tests"))?;
        }
        Ok(())
    }
}

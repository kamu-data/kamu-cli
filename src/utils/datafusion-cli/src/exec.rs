...
pub async fn exec_from_lines(
    ...
    // run the leftover query if the last statement doesn't contain ‘;’
...
async fn create_plan(
    ...
    if let LogicalPlan::Ddl(DdlStatement::CreateExternalTable(cmd)) = &plan {
        register_object_store_and_config_extensions(ctx, &cmd.location, &cmd.options, None).await?;
    }
...
/// and constructs the URL accordingly. Depending on the scheme, it also
/// registers relevant options. The function then alters the default table
/// options with the given custom options. Finally, it retrieves and registers
/// the object store in the session context.
///
/// # Parameters
///
...
mod tests {
    ...
    async fn create_external_table_test(location: &str, sql: &str) -> Result<()> {
        ...
        if let LogicalPlan::Ddl(DdlStatement::CreateExternalTable(cmd)) = &plan {
            register_object_store_and_config_extensions(&ctx, &cmd.location, &cmd.options, None)
                .await?;
        } else {
    ...
    async fn copy_to_external_object_store_test() -> Result<()> {
        ...
        let dialect = dialect_from_str(dialect).ok_or_else(|| {
            plan_datafusion_err!(
                "Unsupported SQL dialect: {dialect}. Available dialects: Generic, MySQL, \
                 PostgreSQL, Hive, SQLite, Snowflake, Redshift, MsSQL, ClickHouse, BigQuery, Ansi."
            )
        })?;
    ...
    async fn copy_to_object_store_table_s3() -> Result<()> {
        ...
        let sql = format!(
            "COPY (values (1,2)) TO '{location}' STORED AS PARQUET
    ...
    async fn create_object_store_table_oss() -> Result<()> {
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET
            OPTIONS('aws.access_key_id' '{access_key_id}', 'aws.secret_access_key' \
             '{secret_access_key}', 'aws.oss.endpoint' '{endpoint}') LOCATION '{location}'"
        );
        create_external_table_test(location, &sql).await?;
    ...
    async fn create_object_store_table_cos() -> Result<()> {
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET
            OPTIONS('aws.access_key_id' '{access_key_id}', 'aws.secret_access_key' \
             '{secret_access_key}', 'aws.cos.endpoint' '{endpoint}') LOCATION '{location}'"
        );
        create_external_table_test(location, &sql).await?;
    ...
    async fn create_object_store_table_gcs() -> Result<()> {
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET OPTIONS('gcp.service_account_key' \
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS PARQUET
            OPTIONS('gcp.application_credentials_path' '{application_credentials_path}') LOCATION \
             '{location}'"
        );
        let err = create_external_table_test(location, &sql)
            .await

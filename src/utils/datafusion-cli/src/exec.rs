    ...
    async fn execute(
        ...
        if physical_plan.boundedness().is_unbounded() {
            if physical_plan.pipeline_behavior() == EmissionType::Final {
                return plan_err!(
                    "The given query can generate a valid result only once the source finishes, \
                     but the source is unbounded"
                );
            }
...
mod tests {
        ...
        let dialect = dialect_from_str(dialect).ok_or_else(|| {
            plan_datafusion_err!(
                "Unsupported SQL dialect: {dialect}. Available dialects: Generic, MySQL, \
                 PostgreSQL, Hive, SQLite, Snowflake, Redshift, MsSQL, ClickHouse, BigQuery, \
                 Ansi, DuckDB, Databricks."
            )
        })?;
    ...
    async fn create_external_table_format_option() -> Result<()> {
        ...
        let sql = format!(
            "CREATE EXTERNAL TABLE test STORED AS CSV LOCATION '{location}' \

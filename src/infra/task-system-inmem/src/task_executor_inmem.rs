...
impl TaskExecutorInMemory {
    ...
    async fn make_dataset_update_result(
        ...
        match pull_result {
            PullResult::UpToDate => Ok(TaskUpdateDatasetResult {
                pull_result,
                num_blocks: 0,
                num_records: 0,
            ...
            PullResult::Updated { old_head, new_head } => {
                self.make_dataset_update_result_from_updated_pull(dataset_id, old_head, new_head)
                    .await
            }
        }
    }
    ...
    async fn make_dataset_update_result_from_updated_pull(
        ...
        let mut block_stream =
            dataset
                .as_metadata_chain()
                .iter_blocks_interval(&new_head, old_head.as_ref(), false);

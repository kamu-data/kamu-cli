    ...
    assert_matches!(
        responses.remove(0),
         PushResponse {
            ...
            result: Ok((SyncResult::Updated { old_head, new_head: _, num_blocks }, _)),
    ...
    assert_matches!(
        responses.remove(0),
         PushResponse {
            local_handle: Some(local_handle),
            target: Some(DatasetPushTarget::Repository(repo_name)),
            result: Ok((SyncResult::UpToDate, _)),
        } if local_handle == foo.dataset_handle &&
            repo_name == harness.remote_repo_name
    );
    ...
    assert_matches!(
        responses.remove(1),
         PushResponse {
            local_handle: Some(local_handle),
            target: Some(DatasetPushTarget::Repository(repo_name)),
            result: Ok((SyncResult::Updated { .. }, _)),
...
impl PushUseCaseHarness {
    ...
    async fn get_remote_aliases(&self, created: &CreateDatasetResult) -> Box<dyn RemoteAliases> {
        self.remote_aliases_registry
            .get_remote_aliases(created.dataset.clone())
            .await
            .unwrap()
    }
}

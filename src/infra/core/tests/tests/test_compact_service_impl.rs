...
impl CompactTestHarness {
    ...
    fn assert_non_add_data_events(
        old_chain: Vec<(Multihash, MetadataBlock)>,
        new_chain: Vec<(Multihash, MetadataBlock)>,
    ) -> bool {
        let mut new_non_add_data_event_index = 0;
        for (old_hash, old_block) in old_chain.iter() {
            if let MetadataEvent::AddData(_) = old_block.event {
                continue;
            }
            let mut old_add_data_event_index = 0;
            for (new_hash, new_block) in new_chain.iter() {

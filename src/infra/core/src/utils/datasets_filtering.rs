...
type FilteredDatasetHandleStream<'a> = Pin<
    Box<
        dyn Stream<Item = Result<odf::DatasetHandle, odf::dataset::GetStoredDatasetError>>
            + Send
            + 'a,
    >,
>;
...
type FilteredDatasetRefAnyStream<'a> = Pin<
    Box<
        dyn Stream<Item = Result<odf::DatasetRefAny, odf::dataset::GetStoredDatasetError>>
            + Send
            + 'a,
    >,
>;
...
) -> impl Stream<Item = Result<odf::DatasetRefAny, odf::dataset::GetStoredDatasetError>> + 'static {
        ...
        for remote_ref_pattern in &remote_ref_patterns {
            ...
            let remote_datasets: Vec<_> = match search_svc.search(None, search_options).await {
                Err(err) => match err {
                    ...
                    _ => Err(odf::dataset::GetStoredDatasetError::Internal(InternalError::new(err)))?,
                },
...
pub fn get_local_datasets_stream<'a>(
    ...
    current_account_name: &odf::AccountName,
) -> impl Stream<Item = Result<odf::DatasetRefAny, odf::dataset::GetStoredDatasetError>> + 'a {
    dataset_registry

...
pub(crate) async fn get_transform_input_from_query_input(
    ...
    let dataset = datasets_map.get_by_id(&query_input.dataset_id);
    let dataset_handle = datasets_map
        .get_handle_by_id(&query_input.dataset_id)
        .clone();
    ...
    let vocab = match vocab_hint {
        Some(v) => v,
        None => get_vocab(dataset.as_ref()).await?,
    };

...
use dill::Catalog;
use kamu_adapter_flow_dataset::{ingest_dataset_binding, FlowScopeDataset, FLOW_TYPE_DATASET_INGEST, FLOW_TYPE_DATASET_TRANSFORM};
use kamu_adapter_flow_webhook::{FlowScopeSubscription, FLOW_TYPE_WEBHOOK_DELIVER};
use kamu_flow_system::*;

use crate::csv_data_loader::CsvFlowProcessStateLoader;
...
pub async fn test_rollup_from_csv_unfiltered(catalog: &Catalog) {
    ...
    // Verify realistic data distribution (9 active, 9 failing, 3 paused_manual, 3 stopped_auto)
...
pub async fn test_rollup_from_csv_filtered_by_scope(catalog: &Catalog) {
    ...
    // Note: Both Dataset scopes and WebhookSubscription scopes match when they have the same dataset_id
    // Row 14 is "acme/orders.daily" which is a different dataset
    ...
    let multi_dataset_query = FlowScopeDataset::query_for_multiple_datasets(&[&dataset_id, &dataset_id2, &dataset_id3]);
    ...
    // beta/catalog (4): rows 9, 10, 17, 24 = 0 active, 2 failing, 0 paused, 2 stopped
    ...
    // Should find webhook subscription processes for acme/orders dataset (rows 5, 6, 7 = 3 total)
    ...
    // Should find only Dataset scope processes for acme/orders (row 1 = 1 total, excludes webhook rows 5, 6, 7)
    assert_eq!(rollup.total, 1);
    ...
    // Should find all webhook subscription processes (rows 5, 6, 7, 8, 9, 10, 15, 16, 19, 20, 22, 24 = 12 total)
    ...
    let multi_webhook_query = FlowScopeSubscription::query_for_subscriptions_of_multiple_datasets(&[&dataset_id, &dataset_id2]);
    ...
    // Test 7: Filter by system flows (should return empty since our CSV has no system flows)
...
pub async fn test_rollup_from_csv_filtered_by_flow_type(catalog: &Catalog) {
    ...
    // Should find 12 webhook processes from CSV data (rows 5, 6, 7, 8, 9, 10, 15, 16, 19, 20, 22, 24)
    assert_eq!(rollup.total, 12);
    assert_eq!(rollup.active, 4); // rows 5, 15, 20, 22
    assert_eq!(rollup.failing, 4); // rows 6, 8, 9, 19
    assert_eq!(rollup.paused, 1); // row 7
...
pub async fn test_rollup_from_csv_filtered_by_effective_status(catalog: &Catalog) {
    ...
    let problematic_states = [FlowProcessEffectiveState::Failing, FlowProcessEffectiveState::StoppedAuto];
    let rollup = flow_process_state_query
...
pub async fn test_rollup_from_csv_combined_filters(catalog: &Catalog) {
    ...
    let rollup = flow_process_state_query
        .rollup_by_scope(FlowScopeQuery::all(), Some(&webhook_flows), Some(&active_states))
        .await
        .unwrap();
    ...
    // Should find only active webhook processes for acme/orders and acme/users datasets
    // From CSV: rows 5 (acme/orders), 20 (acme/users) - both active webhook flows

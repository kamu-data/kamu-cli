...
use kamu_dataset_env_vars_postgres::PostgresDatasetEnvVarRepository;
use sqlx::PgPool;
...
async fn test_missing_dataset_env_var_not_found(pg_pool: PgPool) {
    ...
    DatabaseTransactionRunner::new(harness.catalog)
        .transactional(|catalog| async move {
            kamu_dataset_env_vars_repo_tests::test_missing_dataset_env_var_not_found(&catalog)
                .await;
            Ok::<_, InternalError>(())
        })
        .await
...
async fn test_insert_and_get_dataset_env_var(pg_pool: PgPool) {
    ...
    DatabaseTransactionRunner::new(harness.catalog)
        .transactional(|catalog| async move {
            kamu_dataset_env_vars_repo_tests::test_insert_and_get_dataset_env_var(&catalog).await;
            Ok::<_, InternalError>(())
        })
        .await
        .unwrap();
...
async fn test_delete_dataset_env_vars(pg_pool: PgPool) {
    ...
    DatabaseTransactionRunner::new(harness.catalog)
        .transactional(|catalog| async move {
            kamu_dataset_env_vars_repo_tests::test_delete_dataset_env_vars(&catalog).await;
            Ok::<_, InternalError>(())
        })
        .await
        .unwrap();
...
async fn test_modify_dataset_env_vars(pg_pool: PgPool) {
    ...
    DatabaseTransactionRunner::new(harness.catalog)
        .transactional(|catalog| async move {
            kamu_dataset_env_vars_repo_tests::test_modify_dataset_env_vars(&catalog).await;
            Ok::<_, InternalError>(())
        })
        .await
        .unwrap();

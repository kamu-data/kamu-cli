...
impl PostgresRawResourceEventStore {
    ...
    async fn save_event_rows_multi(
        ...
        let mut resource_ids = Vec::with_capacity(num_total_events);
        let mut resource_schemas = Vec::with_capacity(num_total_events);
    ...
    async fn get_last_event_id(
        ...
        let last_event_id = sqlx::query_scalar!(
            ...
            WHERE
                resource_id = $1 AND resource_schema = $2
...
impl EventStore<ResourceRawEventProjection> for PostgresRawResourceEventStore {
    ...
    fn get_events(
        ...
        Box::pin(async_stream::stream! {
            let mut tr = self.transaction.lock().await;
            let connection_mut = tr.connection_mut().await?;
            let query_id: &uuid::Uuid = key.id.as_ref();

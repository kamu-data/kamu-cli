...
use odf_storage::{GetBlockError, InsertBlockResult, InsertOpts, MetadataBlockRepository};
...
pub fn create_block() -> (odf::MetadataBlock, Bytes, odf::Multihash, u64) {
    let block = odf::MetadataBlock {
        system_time: Utc.with_ymd_and_hms(2050, 1, 1, 12, 0, 0).unwrap(),
        prev_block_hash: None,
        sequence_number: 0,
        event: odf::MetadataEvent::SetInfo(odf::SetInfo {
    ...
    let block_data = FlatbuffersMetadataBlockSerializer
        ...
        .unwrap();
    let hash = odf::Multihash::from_multibase(

...
async fn test_read_only() {
    ...
    assert_matches!(
        ...
        Err(InsertError::Access(odf::AccessError::ReadOnly(_)))
    );
}
...
async fn test_bytes() {
    ...
    let hash_foo = odf::Multihash::from_digest_sha3_256(b"foo");
    let hash_bar = odf::Multihash::from_digest_sha3_256(b"bar");
...
async fn test_stream() {
    ...
    let hash_foobar = odf::Multihash::from_digest_sha3_256(b"foobar");
...
async fn test_external_urls() {
    ...
    let expected_external_upload_url_result = odf::AccessError::ReadOnly(None).into();

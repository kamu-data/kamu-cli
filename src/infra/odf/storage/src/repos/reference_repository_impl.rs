...
{
    async fn get(&self, r: &str) -> Result<odf::Multihash, GetRefError> {
        ...
        let hash = odf::Multihash::from_multibase(text).int_err()?;
        Ok(hash)
    }

    async fn set(&self, r: &str, hash: &odf::Multihash) -> Result<(), SetRefError> {

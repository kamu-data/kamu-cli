...
struct CachedValue {
    block_data: Bytes,
    deserialized_block: Option<odf::MetadataBlock>,
}
...
pub struct MetadataBlockRepositoryCachingInMem<WrappedRepo> {
    wrapped: WrappedRepo,
    cache: DashMap<odf::Multihash, CachedValue>,
}
    ...
    async fn get_block(&self, hash: &odf::Multihash) -> Result<odf::MetadataBlock, GetBlockError> {
        match self.cache.get_mut(hash) {
            Some(mut cached_value) => match &cached_value.deserialized_block {
                ...
                None => {
                    let block =
                        odf_storage::deserialize_metadata_block(hash, &cached_value.block_data)?;
            ...
            None => {
                ...
                match get_block_data_result {
                    Ok(block_data) => {
                        let block = odf_storage::deserialize_metadata_block(hash, &block_data)?;

...
impl ObjectRepository for ObjectRepositoryHttp {
    ...
    async fn contains(&self, hash: &odf::Multihash) -> Result<bool, ContainsError> {
        ...
        match response.error_for_status() {
            ...
            Err(e) if e.status() == Some(http::StatusCode::UNAUTHORIZED) => {
                Err(odf::AccessError::Unauthorized(e.into()).into())
            }
            Err(e) if e.status() == Some(http::StatusCode::FORBIDDEN) => {
                Err(odf::AccessError::Forbidden(e.into()).into())
            }
            Err(e) => Err(e.int_err().into()),
        }
    }

    async fn get_size(&self, _hash: &odf::Multihash) -> Result<u64, GetError> {
    ...
    async fn get_external_upload_url(
        ...
        Err(odf::AccessError::ReadOnly(None).into())
    }
    ...
    async fn delete(&self, _hash: &odf::Multihash) -> Result<(), DeleteError> {
        Err(odf::AccessError::ReadOnly(None).into())
    }
}

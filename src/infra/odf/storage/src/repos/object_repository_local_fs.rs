...
use super::repo_helpers as helpers;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

pub type ObjectRepositoryLocalFSSha3 =
    ObjectRepositoryLocalFS<sha3::Sha3_256, { odf::Multicodec::Sha3_256 as u32 }>;
    ...
    D: digest::Digest,
{
    ...
    async fn write_stream_with_digest<'a>(
        ...
        Ok(odf::Multihash::new(odf::Multicodec::try_from(C).unwrap(), &digest.finalize()).unwrap())
    }
}
    ...
    D: digest::Digest,
{
    ...
    async fn contains(&self, hash: &odf::Multihash) -> Result<bool, ContainsError> {
    ...
    #[tracing::instrument(level = "debug", skip_all, fields(%hash))]
    async fn get_size(&self, hash: &odf::Multihash) -> Result<u64, GetError> {
        let path = self.get_path(hash);
    ...
    #[tracing::instrument(level = "debug", skip_all, fields(%hash))]
    async fn get_bytes(&self, hash: &odf::Multihash) -> Result<Bytes, GetError> {
        let path = self.get_path(hash);
    ...
    #[tracing::instrument(level = "debug", skip_all, fields(%hash))]
    async fn get_stream(&self, hash: &odf::Multihash) -> Result<Box<AsyncReadObj>, GetError> {
        let path = self.get_path(hash);
    ...
    async fn get_internal_url(&self, hash: &odf::Multihash) -> Url {
    ...
    async fn insert_bytes<'a>(
        ...
        let hash = if let Some(hash) = options.precomputed_hash {
            ...
            odf::Multihash::from_digest::<D>(odf::Multicodec::try_from(C).unwrap(), data)
        };
    ...
    #[tracing::instrument(level = "debug", skip_all, fields(%hash))]
    async fn delete(&self, hash: &odf::Multihash) -> Result<(), DeleteError> {
        let path = self.get_path(hash);

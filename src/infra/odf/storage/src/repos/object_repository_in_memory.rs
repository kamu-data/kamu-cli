...
pub struct ObjectRepositoryInMemory {
    blocks_by_hash: Mutex<HashMap<odf::Multihash, Bytes>>,
}
...
impl ObjectRepository for ObjectRepositoryInMemory {
    ...
    async fn get_stream(&self, _hash: &odf::Multihash) -> Result<Box<AsyncReadObj>, GetError> {
        panic!("get_stream not allowed for in-memory repository");
    }

    async fn get_internal_url(&self, _hash: &odf::Multihash) -> Url {
    ...
    async fn insert_bytes<'a>(
        ...
        let hash = if let Some(hash) = options.precomputed_hash {
            ...
            odf::Multihash::from_digest::<sha3::Sha3_256>(odf::Multicodec::Sha3_256, data)
        };

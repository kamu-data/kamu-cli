...
macro_rules! invalid_event {
    ($e:expr, $msg:expr $(,)?) => {
        return Err(AppendValidationError::InvalidEvent(
            odf_dataset::InvalidEventError::new($e, $msg),
        ))
    };
}
    ...
    RefRepo: ReferenceRepository + Sync + Send,
{
    ...
    fn iter_blocks_interval<'a>(
        &'a self,
        head_hash: &'a odf::Multihash,
        tail_hash: Option<&'a odf::Multihash>,
        ...
        Box::pin(async_stream::try_stream! {
            ...
            if !ignore_missing_tail && current.is_none() && tail_hash.is_some() {
                Err(IterBlocksError::InvalidInterval(odf_dataset::InvalidIntervalError {

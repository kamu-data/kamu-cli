...
impl AccountID {
    ...
    pub fn as_bytes(&self) -> AccountIDBytes<'_> {
        AccountIDBytes(self)
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, DeserializeError<Self>> {
        ...
        match key_type {
            Multicodec::Ed25519Pub => DidOdf::from_bytes(bytes)
                .map(Self::Odf)
                .map_err(DeserializeError::new_from),
            Multicodec::MultiDid => unimplemented!("multidid decoding is not yet supported"),
...
impl std::ops::Deref for AccountIDBytes<'_> {
    ...
    fn deref(&self) -> &[u8] {
        // TODO: Consider implementing via `Multicodec::MultiDid` representation
        unimplemented!("AccountID::as_bytes() is not yet implemented")
    }
}
...
impl From<AccountID> for Did {
    fn from(value: AccountID) -> Self {
        match value {
            AccountID::Odf(v) => Did::Odf(v),
            AccountID::Pkh(v) => Did::Pkh(v),
        }
    }
}

...
use super::did_odf::DID_ODF_PREFIX;
...
impl Grammar {
    ...
    pub fn match_resource_name(s: &str) -> Option<(&str, &str)> {
        Self::match_hostname(s)
    }

    pub fn match_resource_type_name(s: &str) -> Option<(&str, &str)> {
        Self::match_subdomain(s)
    }

    // ResourceRef = ResourceTypeName ":"" (AccountName "/")? ResourceName
    pub fn match_resource_ref(s: &str) -> Option<(&str, Option<&str>, &str)> {
        match s.split_once(':') {
            None => None,
            Some((typ, rest)) => {
                ...
                match rest.split_once('/') {
                    None => {
                        let (name, "") = Self::match_resource_name(rest)? else {
                            return None;
                        };
                        Some((typ, None, name))
                    }
                    Some((acc, name)) => {
                        ...
                        let (name, "") = Self::match_resource_name(name)? else {
                            return None;
                        };

...
async fn test_attempt_upload_file_authorized() {
    ...
    let client = async move {
        ...
        let header_map = UploadS3Harness::make_header_map(&upload_context);
        ...
        let actual_file_body =
            UploadS3Harness::read_bucket_file_as_string(&upload_bucket_context, expected_key).await;

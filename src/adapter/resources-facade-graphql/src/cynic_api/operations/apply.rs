...
use kamu_resources_facade::ApplyManifestRequest;
...
impl From<ResourceHeaderValidationProblemCode>
    for kamu_resources_facade::ResourceHeadersValidationProblemCode
{
...
impl From<ResourceInvalidHeaderProblem> for kamu_resources_facade::ApplyManifestError {
    fn from(value: ResourceInvalidHeaderProblem) -> Self {
        Self::InvalidHeaders(kamu_resources_facade::ResourceInvalidHeadersError {
...
impl From<ResourceInvalidSpecProblem> for kamu_resources_facade::ApplyManifestError {
...
impl ResourceApplyOutcome {
    pub(crate) fn try_into_planning_decision(
        self,
    ) -> Result<domain::ApplyManifestPlanningDecision, kamu_resources_facade::ApplyManifestError>
    {
        Ok(match self {
            Self::ResourceApplySuccess(success) => {
                let resource = success
                    .resource
                    .try_into()
                    .map_err(kamu_resources_facade::ApplyManifestError::Internal)?;
    ...
    pub(crate) fn try_into_application_decision(
        self,
    ) -> Result<domain::ApplyManifestApplicationDecision, kamu_resources_facade::ApplyManifestError>
    {
        Ok(match self {
            Self::ResourceApplySuccess(success) => {
                let resource = success
...
) -> Result<kamu_resources_facade::ApplyManifestError, InternalError> {
    use kamu_resources_facade::ParseResourceManifestError;

    use crate::outcome_mapper;

    match outcome {
        ResourceApplyOutcome::ResourceApplyParseManifestProblem(p) => {
            Ok(kamu_resources_facade::ApplyManifestError::ParseManifest(
                ParseResourceManifestError { message: p.message },
            ))
        }
        ResourceApplyOutcome::ResourceUnsupportedDescriptorProblem(p) => Ok(
            kamu_resources_facade::ApplyManifestError::UnsupportedDescriptor(
                outcome_mapper::unsupported_descriptor_problem_error(p),
            ),
        ),
        ResourceApplyOutcome::ResourceAccountResolutionProblem(p) => Ok(
            kamu_resources_facade::ApplyManifestError::AccountResolution(
...
mod tests {
    use super::*;
    use crate::cynic_api::fragments::ResourceManifestFormat;

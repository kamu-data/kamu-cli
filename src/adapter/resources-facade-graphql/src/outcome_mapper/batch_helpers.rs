...
mod tests {
    ...
    fn make_success(index: usize) -> BatchResourceSuccess<()> {
        BatchResourceSuccess {
            request_index: index,
            item: (),
        }
    ...
    fn make_domain_problem(index: usize) -> kamu_resources_facade::BatchResourceProblem<()> {
        kamu_resources_facade::BatchResourceProblem {
            request_index: index,
            error: (),
        }
    }
    ...
    fn validate_batch_response_indexes_duplicate_problem_index_is_error() {
        let successes: Vec<BatchResourceSuccess<()>> = vec![];

...
pub struct DatabaseBackedOdfMetadataChainImpl<TMetadataChain>
where
...
impl<TMetadataChain> DatabaseBackedOdfMetadataChainImpl<TMetadataChain>
where
    TMetadataChain: odf::MetadataChain + Send + Sync,
{
    pub fn new(
...
impl<TMetadataChain> odf::MetadataChain for DatabaseBackedOdfMetadataChainImpl<TMetadataChain>
where
    TMetadataChain: odf::MetadataChain + Send + Sync,
{
    ...
    async fn try_get_prev_block(
        ...
        if let Some(prev_block_hash) = &block.prev_block_hash {
            ...
            if !hint_flags.is_empty() && hint_flags.consists_of_key_blocks_only() {

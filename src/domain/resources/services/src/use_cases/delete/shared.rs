    ...
    R::Status: Serialize + ResourceStatusLike,
{
    ...
    match resource_persistence_service
        .delete(&mut resource, time_source.now())
        .await
    {
        Ok(()) => Ok(()),
...
pub(crate) async fn execute_delete_resources<R>(
    resource_query_service: &dyn ResourceQueryService<R>,
    resource_aggregate_loader: &dyn ResourceAggregateLoader<R>,
    resource_persistence_service: &dyn ResourcePersistenceService<R>,
    time_source: &dyn SystemTimeSource,
    ...
    R::Status: Serialize + ResourceStatusLike,
{
    for resource_id in resource_ids {
        let Some(_resource_snapshot) =
            find_owned_resource_snapshot::<R>(resource_query_service, &account_id, resource_id)
                .await?
        else {
            continue;
        };
    ...
    (
        ...
        impl $crate::domain::DeleteResourcesUseCase<$resource> for $use_case {
            async fn execute(
                ...
                $crate::use_cases::delete::shared::execute_delete_resources::<$resource>(

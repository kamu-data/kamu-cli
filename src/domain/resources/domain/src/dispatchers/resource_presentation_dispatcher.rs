...
pub trait ResourcePresentationDispatcher: Send + Sync {
    ...
    /// results of a *single* scoped query. The typed alternative —
    /// [`ResourceCrudDispatcher::list`] — runs its own paginated query per
    /// type, which cannot produce correct global pagination across types:
    ...
    /// wrong columns.
    ///
    /// [`ResourceCrudDispatcher::list`]: crate::ResourceCrudDispatcher::list

...
| RF-105  | list_search.rs      | Active   | Per-selector `account` is rejected, not ignored                       | ODF defines it and the GraphQL input exposes it, but listing resolves one account per call. Rejected so it fails loudly instead of silently scoping to the caller's own account. Covers the type-less fast path too |
| RF-106  | list_search.rs      | Active   | Type-less selector scope limits (`UnrepresentableScopeError`)         | The two `AnyType` limits — mixed with typed selectors, and multiple query modes. Asserted on message, not variant: remote surfaces them as transport errors. Both disappear in the per-row-type stage |

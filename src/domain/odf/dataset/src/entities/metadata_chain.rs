...
pub trait MetadataChain: Send + Sync {
    ...
    fn iter_blocks_interval<'a>(
        &'a self,
        head: &'a odf::Multihash,
        tail: Option<&'a odf::Multihash>,
...
pub trait MetadataChainExt: MetadataChain {
    ...
    async fn try_get_ref(&self, r: &BlockRef) -> Result<Option<odf::Multihash>, InternalError> {
    ...
    async fn get_block_by_ref(&self, r: &BlockRef) -> Result<odf::MetadataBlock, InternalError> {
    ...
    async fn try_get_block(
        &self,
        hash: &odf::Multihash,
    ) -> Result<Option<odf::MetadataBlock>, InternalError> {
    ...
    async fn accept_by_interval<E>(
        &self,
        visitors: &mut [&mut dyn MetadataChainVisitor<Error = E>],
        head_hash: Option<&odf::Multihash>,
        tail_hash: Option<&odf::Multihash>,
    ) -> Result<(), AcceptVisitorError<E>>
    where
        E: Error + Send,
    {
            ...
            for (decision, visitor) in decisions.iter_mut().zip(visitors.iter_mut()) {
                match decision {
                    ...
                    MetadataVisitorDecision::NextOfType(requested_flags) => {
                        let block_flag = odf::MetadataEventTypeFlags::from(&block.event);
    ...
    where
        S: Send,
        F: Fn(&mut S, &odf::Multihash, &odf::MetadataBlock) -> MetadataVisitorDecision + Send,
    {
    ...
    where
        S: Send,
        F: Send,
        F: Fn(&mut S, &odf::Multihash, &odf::MetadataBlock) -> MetadataVisitorDecision,
    {
    ...
    where
        ...
        F: Send,
        E: Error + Send,
        F: Fn(&mut S, &odf::Multihash, &odf::MetadataBlock) -> Result<MetadataVisitorDecision, E>,
    {
        let head_hash = self
...
pub struct SetRefOpts<'a> {
    ...
    /// (compare-and-swap)
    pub check_ref_is: Option<Option<&'a odf::Multihash>>,
}
...
pub struct AppendOpts<'a> {
    ...
    /// hash.
    pub precomputed_hash: Option<&'a odf::Multihash>,

    /// Append will result in error if computed hash does not match this one.
    pub expected_hash: Option<&'a odf::Multihash>,
}
...
pub struct RefCASError {
    ...
    pub expected: Option<odf::Multihash>,
    pub actual: Option<odf::Multihash>,
}
...
impl AppendValidationError {
    pub fn no_op_event(event: impl Into<odf::MetadataEvent>, message: impl Into<String>) -> Self {
        Self::NoOpEvent(NoOpEventError::new(event, message))
    }

    pub fn empty_event(event: impl Into<odf::MetadataEvent>) -> Self {
...
pub struct InvalidEventError {
    event: Box<odf::MetadataEvent>,
    message: String,
}

impl InvalidEventError {
    pub fn new(event: impl Into<odf::MetadataEvent>, message: impl Into<String>) -> Self {
        Self {

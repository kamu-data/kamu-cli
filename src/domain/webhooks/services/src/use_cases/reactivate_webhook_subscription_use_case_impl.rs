// Copyright Kamu Data, Inc. and contributors. All rights reserved.
//
// Use of this software is governed by the Business Source License
// included in the LICENSE file.
//
// As of the Change Date specified in that file, in accordance with
// the Business Source License, use of this software will be governed
// by the Apache License, Version 2.0.

use std::sync::Arc;

use dill::{component, interface};
use kamu_webhooks::*;
use messaging_outbox::{Outbox, OutboxExt};

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#[component(pub)]
#[interface(dyn ReactivateWebhookSubscriptionUseCase)]
pub struct ReactivateWebhookSubscriptionUseCaseImpl {
    subscription_event_store: Arc<dyn WebhookSubscriptionEventStore>,
    outbox: Arc<dyn Outbox>,
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#[common_macros::method_names_consts]
#[async_trait::async_trait]
impl ReactivateWebhookSubscriptionUseCase for ReactivateWebhookSubscriptionUseCaseImpl {
    #[tracing::instrument(
        level = "info",
        name = ReactivateWebhookSubscriptionUseCaseImpl_execute,
        skip_all,
        fields(subscription_id=%subscription.id()),
    )]
    async fn execute(
        &self,
        subscription: &mut WebhookSubscription,
    ) -> Result<(), ReactivateWebhookSubscriptionError> {
        let old_status = subscription.status();

        subscription
            .reactivate()
            .map_err(|e: ProjectionError<WebhookSubscriptionState>| {
                tracing::error!(error=?e, error_msg=%e, "Webhook subscription reactivate failed");
                ReactivateWebhookSubscriptionError::ReactivateUnexpected(
                    ReactivateWebhookSubscriptionUnexpectedError {
                        status: subscription.status(),
                    },
                )
            })?;

        subscription
            .save(self.subscription_event_store.as_ref())
            .await
            .map_err(|e| ReactivateWebhookSubscriptionError::Internal(e.int_err()))?;

        let latest_status = subscription.status();

        if old_status != latest_status {
            for event_type in subscription.event_types() {
                self.outbox
                    .post_message(
                        MESSAGE_PRODUCER_KAMU_WEBHOOK_SUBSCRIPTION_EVENT_CHANGES_SERVICE,
                        WebhookSubscriptionEventChangesMessage::event_enabled(
                            subscription.id(),
                            subscription.dataset_id(),
                            event_type.clone(),
                        ),
                    )
                    .await?;
            }
        }

        Ok(())
    }
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

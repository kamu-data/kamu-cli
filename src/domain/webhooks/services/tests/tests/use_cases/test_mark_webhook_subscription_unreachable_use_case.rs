...
use dill::*;
use kamu_webhooks::{
    MarkWebhookSubscriptionUnreachableError,
    MarkWebhookSubscriptionUnreachableUseCase,
    WebhookEventTypeCatalog,
    WebhookSubscriptionStatus,
};
use kamu_webhooks_services::MarkWebhookSubscriptionUnreachableUseCaseImpl;
...
async fn test_mark_unreachable_success() {
    let foo_id = odf::DatasetID::new_seeded_ed25519(b"foo");

    let mut mock_outbox = MockOutbox::new();
    WebhookSubscriptionUseCaseHarness::expect_webhook_subscription_marked_unreachable_message(
        &mut mock_outbox,
    ...
    let harness = MarkWebhookSubscriptionUnreachableUseCaseHarness::new(mock_outbox);
    ...
    assert!(
        res.is_ok(),
        "Failed to mark subscription as unreachable: {res:?}",
    );
    assert_eq!(
        subscription.status(),
...
#[test_log::test(tokio::test)]
async fn test_mark_unreachable_idempotence() {
    let foo_id = odf::DatasetID::new_seeded_ed25519(b"foo");
...
#[test_log::test(tokio::test)]
async fn test_mark_unreachable_unexpected() {
    let foo_id = odf::DatasetID::new_seeded_ed25519(b"foo");

    let harness = MarkWebhookSubscriptionUnreachableUseCaseHarness::new(MockOutbox::new());
    let mut subscription = harness.create_subscription_in_dataset(foo_id).await;

    let res = harness.use_case.execute(&mut subscription).await;
    assert_matches!(
        res,
        Err(MarkWebhookSubscriptionUnreachableError::MarkUnreachableUnexpected(e))
...
struct MarkWebhookSubscriptionUnreachableUseCaseHarness {
    base_harness: WebhookSubscriptionUseCaseHarness,
    use_case: Arc<dyn MarkWebhookSubscriptionUnreachableUseCase>,
}

impl MarkWebhookSubscriptionUnreachableUseCaseHarness {

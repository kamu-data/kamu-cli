...
use crate::{Dataset, DatasetHandleStream, GetDatasetError, ResolvedDataset};
...
pub trait DatasetRegistryExt: DatasetRegistry {
    ...
    fn get_resolved_dataset_by_handle(&self, dataset_handle: &DatasetHandle) -> ResolvedDataset;

    async fn get_resolved_dataset_by_ref(
        &self,
        dataset_ref: &DatasetRef,
    ...
    T: ?Sized,
{
    ...
    async fn get_resolved_dataset_by_ref(
        ...
        let dataset_handle = self.resolve_dataset_handle_by_ref(dataset_ref).await?;
        Ok(self.get_resolved_dataset_by_handle(&dataset_handle))
    }
}

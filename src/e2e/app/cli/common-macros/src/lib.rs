...
#[proc_macro]
pub fn kamu_cli_run_api_server_e2e_test_elasticsearch(input: TokenStream) -> TokenStream {
    let harness_method = parse_str("run_api_server").unwrap();

    kamu_cli_e2e_test_elasticsearch_impl(&harness_method, input)
}
...
fn kamu_cli_e2e_test_elasticsearch_impl(harness_method: &Ident, input: TokenStream) -> TokenStream {
    let InputArgsElasticsearch {
        storage,
        fixture,
        options,
        extra_test_groups,
        elasticsearch_config,
    } = parse_macro_input!(input as InputArgsElasticsearch);
    ...
    let output = match storage.to_string().as_str() {
        "postgres" => quote! {
            #[test_group::group(e2e, database, postgres, elasticsearch, #extra_test_groups)]
            #[test_log::test(sqlx::test(migrator = "database_common::POSTGRES_MIGRATOR"))]
            async fn #test_function_name (pg_pool: sqlx::PgPool) {
                let es_config = #elasticsearch_config;
                let options = #options;
                let options = options.with_elasticsearch_test_cleanup(es_config.index_prefix);
        ...
        "mysql" => quote! {
            #[test_group::group(e2e, database, mysql, elasticsearch, #extra_test_groups)]
            #[test_log::test(sqlx::test(migrator = "database_common::MYSQL_MIGRATOR"))]
            async fn #test_function_name (mysql_pool: sqlx::MySqlPool) {
                let es_config = #elasticsearch_config;
                let options = #options;
        ...
        "sqlite" => quote! {
           // kamu-cli will create sqlite database by itself and apply migrations to it
           #[test_group::group(e2e, elasticsearch, #extra_test_groups)]
           #[test_log::test(tokio::test)]
           async fn #test_function_name () {
               let es_config = #elasticsearch_config;
               let options = #options;
...
struct InputArgsElasticsearch {
    pub storage: Ident,
    pub fixture: Path,
    pub options: Option<Expr>,
    pub extra_test_groups: Option<LitStr>,
    pub elasticsearch_config: Expr,
}
...
impl Parse for InputArgsElasticsearch {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        let mut storage = None;
        let mut fixture = None;
        let mut options = None;
        let mut extra_test_groups = None;
        let mut elasticsearch_config = None;
        ...
        while !input.is_empty() {
            ...
            match key.to_string().as_str() {
                ...
                "elasticsearch_config" => {
                    ...
                    elasticsearch_config = Some(value);
                }
                unexpected_key => panic!(
                    "Unexpected key: {unexpected_key}\nAllowable values: \"storage\", \
                     \"fixture\", \"options\", \"extra_test_groups\", and \
                     \"elasticsearch_config\"."
                ),
            }
        ...
        let Some(elasticsearch_config) = elasticsearch_config else {
            panic!("Mandatory parameter \"elasticsearch_config\" not found");
        };

        Ok(InputArgsElasticsearch {
            storage,
            fixture,
            options,
            extra_test_groups,
